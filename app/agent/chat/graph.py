...
from app.agent.chat.nodes import trim_messages, make_call_model, format_response, make_log_memory, make_inject_context, make_check_report_coverage
...
def build_workflow(llm: BaseChatModel, checkpointer: BaseCheckpointSaver):
    ...
    builder.add_node("check_report_coverage", make_check_report_coverage(store))
    builder.add_node("call_model", make_call_model(llm, profile, chatbot_tools, respond_tool))
    ...
    builder.add_edge("inject_context", "check_report_coverage")
    builder.add_edge("check_report_coverage", "call_model")
